...
const tools = [
  ...
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
        <path d="M4 6H18M4 11H13M4 16H16" stroke="#0F0F0F" strokeWidth="1.3" strokeLinecap="round"/>
  ...
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
        <path d="M4 18L4 12M8 18L8 8M12 18L12 10M16 18L16 5" stroke="#0F0F0F" strokeWidth="1.3" strokeLinecap="round"/>
  ...
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
        <rect x="4" y="7" width="14" height="10" rx="2" stroke="#0F0F0F" strokeWidth="1.3"/>
        <path d="M7 7V6C7 4.895 7.895 4 9 4H13C14.105 4 15 4.895 15 6V7" stroke="#0F0F0F" strokeWidth="1.3"/>
...
const tagColors = {
  Investing: "bg-[#F5F1E8] text-[#8B7340] border-[#E8DFC0]",
  Savings: "bg-[#F0F5FF] text-[#4A6FA5] border-[#D0E0FF]",
  Budgeting: "bg-[#F0FBF4] text-[#3A7A5A] border-[#C0E8D0]",
  Planning: "bg-[#F8F0FF] text-[#7A4AA0] border-[#E0C8F8]",
  Debt: "bg-[#FFF0F0] text-[#A04A4A] border-[#F8C8C8]",
  Goals: "bg-[#F5F5F3] text-[#555] border-[#E0E0E0]",
};
...
function ToolCard({ tool, delay }) {
  ...
  return (
    <div
      ref={ref}
      className="group p-6 rounded-2xl border border-[#E8E8E8] bg-white hover:border-[#D0D0D0] hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(0,0,0,0.08)] transition-all duration-200 cursor-pointer"
    >
      <div className="flex items-start justify-between mb-4">
        <div className="w-10 h-10 rounded-xl bg-[#F8F8F6] border border-[#F0F0F0] flex items-center justify-center group-hover:bg-[#F5F1E8] transition-colors duration-200">
          {tool.icon}
        </div>
      ...
      <h3 className="text-[15px] font-semibold text-[#0F0F0F] tracking-[-0.02em] mb-2">{tool.name}</h3>
      <p className="text-[13.5px] leading-relaxed text-[#777] tracking-[-0.01em]">{tool.desc}</p>
      <div className="mt-5 flex items-center gap-1 text-[12.5px] font-medium text-[#888] group-hover:text-[#0F0F0F] transition-colors duration-200">
        Open tool
        ...
        <div ref={headerRef} className="flex flex-col md:flex-row md:items-end justify-between mb-14 gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-[#E8E8E8] mb-5">
              <span className="text-[12px] text-[#666] font-medium tracking-wide">Finance Tools</span>
            </div>
            <h2
              className="text-[38px] font-semibold tracking-[-0.03em] text-[#0F0F0F] leading-tight max-w-sm"
              style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}
            >
              Powerful calculators at your fingertips
            </h2>
          </div>
          <p className="text-[15px] text-[#666] max-w-xs leading-relaxed tracking-[-0.01em] md:text-right">
